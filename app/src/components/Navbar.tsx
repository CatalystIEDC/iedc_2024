...
const Navbar = () => {
    ...
    return (
        <div className='flex fixed z-[300] top-0 w-screen py-4 px-8 lg:px-14 justify-between bg-[#010314] backdrop-blur-2xl items-center overflow-hidden text-white'>
            ...
            <HiOutlineMenuAlt3 onClick={toggleNavbar} size={30} className='lg:hidden' />

            {/* Mobile Menu */}
            <div className={`fixed top-0 right-0 h-full w-3/4 bg-[#010314] transform ${isOpen ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-300 ease-in-out lg:hidden`}>
                <div className='flex flex-col items-center mt-20'>
                    {
                        links.map((link, index) => {
                            return (
                                <Link key={index} className="text-base font-light text-white my-4" href={link.path} onClick={toggleNavbar}>{link.page}</Link>
                            )
                        })
                    }
                </div>
